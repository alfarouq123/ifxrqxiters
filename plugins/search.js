...
module.exports = async (req, res) => {
  ...
  if (!message?.trim()) {
    return res.status(400).json({ error: { message: 'query kosong' } });
  }
  ...
  try {
    const upstream = await fetch(
      `https://api.synoxcloud.xyz/ai-chat/copilot-ai?message=${encodeURIComponent(message)}&model=${encodeURIComponent(model)}`,
      {
    ...
    try {
      ...
      return res.status(502).json({ error: { message: 'response api bukan json' }, raw: text });
    }

...
async function runEdit(imageUrlOrBase64, mimeType, prompt) {
  if(!imageUrlOrBase64) { const e = new Error('gambar kosong'); e.status = 400; throw e; }
  if(!prompt)           { const e = new Error('prompt kosong'); e.status = 400; throw e; }

  // terima imageUrl langsung (dari /api/upload proxy) ATAU base64 (fallback)
  let publicUrl;
  if(imageUrlOrBase64.startsWith('http')) {
    publicUrl = imageUrlOrBase64;
  } else {
    const buffer = Buffer.from(imageUrlOrBase64, 'base64');
    const ext    = ((mimeType || 'image/jpeg').split('/')[1] || 'jpg').split('+')[0];
    publicUrl    = await uploadToUguu(buffer, 'ifxrq-' + Date.now() + '.' + ext, mimeType || 'image/jpeg');
  }
  ...
  try {
    ...
    const err = new Error('koneksi AI gagal: ' + e.message); err.status = 502; throw err;
  }

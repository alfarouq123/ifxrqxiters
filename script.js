async function sendMessage() {
    ...
    // Ambil API Key dari browser
    let apiKey = localStorage.getItem("gemini_key");
    if (!apiKey) {
        apiKey = prompt("Masukin API Key Gemini lo (Hanya disimpan di browser ini):");
    ...
    // Personalisasi AI (System Prompt)
    ...
    try {
        ...
        if (data.candidates && data.candidates[0].content.parts[0].text) {
            ...
            // Kalau model 3.1-flash-lite belum tersedia, dia bakal kasih error di sini
            chatBox.innerHTML += `<div class="msg if">Error: ${data.error ? data.error.message : "Model tidak ditemukan. Coba ganti ke gemini-1.5-flash."}</div>`;
        }

// api/play.js — route tipis, logic di plugins/play.js
const { searchYoutube, downloadAudio, downloadVideo, fmtDuration, fmtViews } = require('../plugins/play');

module.exports = async (req, res) => {
  if (req.method !== 'GET') {
    return res.status(405).json({ error: { message: 'pake GET' } });
  }

  const { action, query, url } = req.query;
  // action: 'search' | 'audio' | 'video'

  try {
    if (action === 'search') {
      if (!query) return res.status(400).json({ error: { message: 'query kosong' } });
      const info = await searchYoutube(query);
      return res.status(200).json({ ok: true, ...info, durationFmt: fmtDuration(info.duration), viewsFmt: fmtViews(info.views) });
    }

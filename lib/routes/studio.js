...
const VIDEO_MODELS = {
  'ifz-low':    { label: 'IFz Low',    durations: [4, 6, 8],    desc: 'cepat & hemat — buat eksperimen', chain: ['veo-3.0-fast-generate-001'] },
  'ifz-lite':   { label: 'IFz Lite',   durations: [4, 6, 8],    desc: 'kualitas seimbang, generate cepat', chain: ['veo-3.1-fast-generate-preview', 'veo-3.0-fast-generate-001'] },
  'ifz-ultra':  { label: 'IFz Ultra',  durations: [8],          desc: 'kualitas tertinggi — detail & sinematik', chain: ['veo-3.1-generate-preview', 'veo-3.0-generate-001'] },
  'ifz-stable': { label: 'IFz Stable', durations: [4, 6, 8, 10], desc: 'stabil 720p, durasi paling fleksibel', chain: ['veo-3.0-generate-001'] },
};
const IMAGE_MODELS = {
  'laps-lite':  { label: 'Laps Lite',  desc: 'hasil cepat, buat iterasi ide', chain: ['gemini-2.5-flash-image'] },
  'laps-fast':  { label: 'IFz Laps Fast', desc: 'nano banana 2 — detail bagus', chain: ['gemini-3-pro-image-preview', 'gemini-2.5-flash-image'] },
  'laps-ultra': { label: 'Laps Ultra', desc: 'nano banana 2 pro — kualitas max', chain: ['gemini-3-pro-image-preview', 'gemini-2.5-flash-image'] },
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'veo-start') {
      ...
      const dur = Number(seconds) || cfg.durations[0];
      const data = await gemChain(cfg.chain, (m) => ({
        ...
        opts: {
          ...
          body: JSON.stringify({
            instances: [{ prompt: p }],
            parameters: { aspectRatio: aspect === '9:16' ? '9:16' : '16:9', durationSeconds: dur },
          }),
        },
      }));
      if (!data?.name) throw new Error('gemini gak balikin operation id');
      return res.status(200).json({ ok: true, op: data.name });
    }

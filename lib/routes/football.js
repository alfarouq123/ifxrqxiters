  ...
  try {
    ...
    if (body.action === 'matchReady') {
      await withDbWrite(async (db) => {
        ...
        if (m) { if (m.a.uid === uid) m.readyA = true; if (m.b.uid === uid) m.readyB = true; }
        return true;
      });
      const { db } = await readDb();
    ...
    if (body.action === 'matchReadyPre') {
      ...
      await withDbWrite(async (db) => {
        getFb(db);
        const m = db.football._matches[String(body.mid || '')];
        if (m) { if (m.a.uid === uid) m.readyPreA = true; if (m.b.uid === uid) m.readyPreB = true; }
        return true;
      });
      const { db } = await readDb();
      const m = db.football._matches[String(body.mid || '')];
    ...
    if (body.action === 'roomReady') {
      await withDbWrite(async (db) => {
        ...
        const r = db.football._rooms[String(body.roomId || '').toUpperCase()];
        if (r) {
          if (r.host.uid === uid) { r.readyA = true; if (body.squad) r.host.squad = body.squad; }
          if (r.guest && r.guest.uid === uid) { r.readyB = true; if (body.squad) r.guest.squad = body.squad; }
        }
        return true;
      });
      const { db } = await readDb();
    ...
    if (body.action === 'roomReadyHalf') {
      ...
      await withDbWrite(async (db) => {
        getFb(db);
        const r = db.football._rooms[String(body.roomId || '').toUpperCase()];
        if (r) { if (r.host.uid === uid) r.readyHA = true; if (r.guest && r.guest.uid === uid) r.readyHB = true; }
        return true;
      });
      const { db } = await readDb();
      const r = db.football._rooms[String(body.roomId || '').toUpperCase()];

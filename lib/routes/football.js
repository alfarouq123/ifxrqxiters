...
/* target/divisi info dikirim ke frontend */
function divMeta(fb){
  if (fb.division >= 4) return { type:'points', division:fb.division, points:fb.points, target:TARGET[fb.division]||20, stay:STAY_PT[fb.division]||10, matches:fb.matches, season:SEASON_MATCHES };
  return { type:'rating', division:fb.division, rating:fb.rating, upAt:RATE_UP[fb.division]||null, floor:RATE_FLOOR[fb.division]||1000 };
}
function upsertLeaderboard(fb, uid, name){
  ...
  const entry = { uid, name:String(name||'Player').slice(0,24), division:fb[uid].division, points:fb[uid].points, rating:fb[uid].rating, w:fb[uid].w, ts:Date.now() };
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (body.action === 'report') {
      ...
      await withDbWrite(async (db) => {
        ...
        if ((body.mode === 'ai' || body.mode === 'pvp') && fb.division >= 4) {
          ...
          fb.points += outcome === 'w' ? 3 : outcome === 'd' ? 1 : 0;
          fb.matches++;
          if (fb.points >= (TARGET[fb.division] || 20)) {
            promo = 'up';                                   /* sentuh target → promosi langsung */
            fb.division = Math.max(1, fb.division - 1);
            fb.points = 0; fb.matches = 0;
            if (fb.division < 4) fb.rating = RATE_FLOOR[fb.division] || 1000;
          } else if (fb.matches >= SEASON_MATCHES) {        /* musim habis tapi belum target */
            if (fb.points >= (STAY_PT[fb.division] || 10)) { promo = 'stay'; }   /* stay */
            else { promo = 'down'; fb.division = Math.min(10, fb.division + 1); } /* degradasi */
            fb.points = 0; fb.matches = 0;
          }
          ...
          const floor = RATE_FLOOR[fb.division] || 1000;
          if (fb.division === 3 && fb.rating >= RATE_UP[3]) { promo = 'up'; fb.division = 2; }
          else if (fb.division === 2 && fb.rating >= RATE_UP[2]) { promo = 'up'; fb.division = 1; }
          const newFloor = RATE_FLOOR[fb.division] || 1000;
          if (fb.rating < newFloor) fb.rating = newFloor;   /* gak turun di bawah dasar divisi */
        }
        upsertLeaderboard(db.football, uid, nm);
        return true;
      });
      const { db } = await readDb();
      return res.status(200).json({ ok: true, state: getFb(db), promo, meta: divMeta(getFb(db)) });
    }
    ...
    if (body.action === 'leaderboard') {
      ...
      const topPoints = lb.filter((e)=>e.division>=4).sort((a,b)=> (b.division-a.division) || (b.points-a.points)).slice(0,20);
      const topRating = lb.filter((e)=>e.division<4).sort((a,b)=> b.rating-a.rating).slice(0,20);
      const topWins = lb.slice().sort((a,b)=> b.w-a.w).slice(0,20);
      return res.status(200).json({ ok:true, points:topPoints, rating:topRating, wins:topWins });
    }

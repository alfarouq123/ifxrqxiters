...
async function x2twitterDl(url) {
  try {
    ...
    /* tweet tanpa video / di-private → API balas statusCode 404 tanpa html — jangan crash */
    if (searchResponse.data.statusCode && searchResponse.data.statusCode !== 200) {
      return { error: true, message: searchResponse.data.msg || 'Video tidak ditemukan di tweet itu (mungkin bukan video atau di-private).' };
    }

    const html = searchResponse.data.data;
    if (!html || typeof html !== 'string') {
      return { error: true, message: 'Video tidak ditemukan di tweet itu (mungkin bukan video atau di-private).' };
    }
